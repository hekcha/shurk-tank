...
export const Sidebar = ({ isopen, toggle }) => {
  ...
  return (
    ...
    >
      <div className="icon">
        <span onClick={toggle}>
          <svg
            ...
            className="hover:[stroke:gray] [stroke:#000]"
            style={{ strokeWidth: '3.5', strokeLinecap: 'round' }}
          >
      ...
      <div className="sidebar-wrapper h-[60%] justify-start self-start my-[10%]">
        <div className="sidebar-menu h-full flex flex-col justify-evenly	">
          {sidebarItems.map((item, index) => {
            return (
              <a className="text-[40px] sidebar-link" href={item.link} key={index}>
                {item.name}
              </a>
            );
          })}
